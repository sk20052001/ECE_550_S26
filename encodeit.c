...
static inline long pick_immediate(short mov_size)
{
	...
	case ISZ_4: return (long)(((unsigned long long)rand() << 16) ^ (unsigned long long)rand());
	case ISZ_8:
	default:
...
static void sync_barrier(volatile int *counter, int nthreads)
{
	...
	while (*counter < nthreads) { }    /* spin until everyone is here */
}
...
int main(int argc, char *argv[])
{
	...
	if (mdptr == (volatile char *)MAP_FAILED) { perror("Couldn't mmap DATA"); exit(1); }
	...
	if (mptr == (volatile char *)MAP_FAILED) { perror("Couldn't mmap CODE"); exit(1); }
	...
	if (comm_ptr == (volatile char *)MAP_FAILED) { perror("Couldn't mmap COMM"); exit(1); }
